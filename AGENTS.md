...
**Toolchain:** `node core/dist/cli/index.js <intake|decompose|compare|plan|board|conform|creative-check|review-validate|review-score|review-calibrate|validate|check|render|evidence|release|verify-release|probe>` (build once with `cd core && npm install && npx tsc`). Requires ffmpeg on PATH.
