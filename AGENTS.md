...
**Toolchain:** `node core/dist/cli/index.js <decompose|validate|check|render|evidence|probe>` (build once with `cd core && npm install && npx tsc`). Requires ffmpeg on PATH.
