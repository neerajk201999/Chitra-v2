...
| P1 | A release asset can be replaced under the same filename by a privileged maintainer. | The release, docs, and benchmark pin SHA-256 `d8bc89…670ba9`; changed bytes fail before installation. GitHub's asset metadata reports the same digest. |
...
- GitHub release `v0.5.0-rc.1` is marked prerelease and targets protected-main
  commit `90cd93ee2fc36ef6b1292bc07364c68c345edb40`.
- GitHub reports asset size 586,068 bytes and the pinned SHA-256.
- First public run: 3.9s install, 62.9 MiB, zero browser bytes, 11.7s through a
  1,709 KiB browser frame with warm dependencies.
- Repeated run: 1.5s install and 6.6s complete with the same digest and output
  frame size. The lower repeat is not substituted for the conservative first run.
