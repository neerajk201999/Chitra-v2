...
5. **Expressiveness ceiling (partly closed)**: figures, video-in-scene, cursor/type choreography, particles, real 3D primitives, and reason-gated frame-addressed transform tracks are built (ADR-0007…0013). Card Vault clean-room 0.5 proves all 274 frames can be authored, but not exactly: the final ring cannot target a custom dot constellation, the hero card lacks internal 3D/light tracks, and the phone collapse lacks motion blur. Masks/mattes, general nested compositions, blend modes, and richer motif vocabulary also remain typed-IR gaps.
