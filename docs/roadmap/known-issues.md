...
5. **Expressiveness ceiling**: no keyframes, masks, video-in-scene, nested compositions, per-character type animation; one chart type. Images now first-class (ADR-0006: fetch/snap acquisition, content-hashed, MO-MED gated); video references handled as extracted stills only. Raised only via curated presets/elements, never raw escape hatches (M3+).
