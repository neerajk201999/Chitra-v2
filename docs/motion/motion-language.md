...
- IR-GROUP-1: a one-level `group` may own sibling elements as a full-stage parent transform context; every child has exactly one owner, and missing, multiply-owned, or group children are P1. *(schema + static + compiler — ADR-0021)*
