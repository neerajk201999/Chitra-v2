...
- MO-PART-1: particle fields are bounded (≤ 400 dots) and ambient by default; shimmer opacity floors at 0.22 (dots never fully vanish); `morphTo` only on `particle-morph`. Formations: grid / ring / scatter (seeded) or an ordered custom constellation of normalized in-bounds points; custom morph source/destination counts must match. *(P1, schema + static — ADR-0009/0020)*
