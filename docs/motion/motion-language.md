...
- MO-CHOR-3: transform composition is compiler-managed; authors use curated intent presets by default. Frame-addressed transform tracks are the explicit, reasoned exception in MO-KEY-1.
- IR-GROUP-1: a `group` establishes a bounded local coordinate/compositing
  context and may own nested groups to depth eight; every child has exactly one
  owner, and missing, duplicate, multiply-owned, cyclic, or over-depth
  hierarchies are P1. *(schema + static + compiler — ADR-0021/0042)*
...
- MO-PART-1: particle field structure must be valid: shimmer opacity floors at 0.22 (dots never fully vanish); `morphTo` only appears on `particle-morph`; custom formations have normalized in-bounds points; custom morph source/destination counts match. *(P1 hard defect, schema + static — ADR-0009/0020)*
- MO-PART-2: fields above 400 dots require a visible performance/density review; high density is permitted when intentional. *(P1 style flag, static — ADR-0009/0020/0041)*
...
- MO-AUD-5: `at.onNarrationWord` must cite one declared word whose absolute
  start occurs inside the animation's scene. Word-addressed motion shares the
  frozen narration clock; missing or cross-scene cues are hard errors.
  *(P1, static — ADR-0044)*
...
- MO-3D-2: exact internal 3D tracks use `three-keyframe-track` on one `scene3d` target, begin at relative frame 0, remain within their scene, use only typed mesh/camera/key-light/fill-light/exposure properties and token easings, and carry `override.reason`. The final frame sets duration; incompatible preset controls and multiple internal-track owners are forbidden. A DOM `keyframe-track` may independently animate the containing canvas. *(P1, schema + static — ADR-0028)*

- MO-KEY-1: exact transform tracks use `keyframe-track`, begin at relative frame 0, remain within their scene, use only typed properties/token easings, and carry `override.reason` explaining why a curated preset is insufficient. A track's final frame determines duration from the score FPS; it never mixes duration, distance, direction, stagger, waypoint, morph, or raw-easing controls. *(P1, schema + static — ADR-0013)*
...
- MO-TYPE-5: non-bundled brand fonts are project-local WOFF2 faces declared at
  the exact used weight with source provenance; bytes are embedded offline and
  participate in cache/release identity. Remote fonts and silent fallback are
  forbidden. *(P1, schema + compiler + conformance — ADR-0040)*
